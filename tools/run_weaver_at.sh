...
start_iin_agents() {
    ...
    # Fix host file mode 640 -> world-readable so the iinagent user inside the
    # container can open the bind-mounted dnsconfig.json / config.json /
    # security-domain-config.json. Without this, the agent fails membership
    # sync with: EACCES: permission denied, open '/opt/iinagent/dnsconfig.json'.
    chmod a+rX docker-testnet/configs/ 2>/dev/null || true
    chmod a+r docker-testnet/configs/*.json 2>/dev/null || true

    # Same class of fix for the peer-org tree mounted at /opt/iinagent/extra.
    # The makefile's deploy target chmods only the top-level dir (777); files
    # inside (e.g. connection-org1.docker.json) keep mode 640 and would still
    # be unreadable by the iinagent uid. Recursively add 'a+rX' so files are
    # world-readable and traversable directories stay traversable.
