...
if [[ $local == "local" ]]; then
  ...
  if [ -f $file ]; then
    ...
    (cd $directory/../shared/artifacts && curl --location -u $username:$password -O $baseUrl/corda/app/interop/interop-contracts/$cactiVersion/interop-contracts-$cactiVersion.jar) || exit 1
    (cd $directory/../shared/artifacts && curl --location -u $username:$password -O $baseUrl/corda/app/interop/interop-workflows/$cactiVersion/interop-workflows-$cactiVersion.jar) || exit 1
    (cd $directory/../shared/artifacts && curl --location -u $username:$password -O $baseUrl/protos-java-kt/$cactiVersion/protos-java-kt-$cactiVersion.jar) || exit 1
  else
    echo Please copy the github.properties.template file as github.properties and replace placeholders with Github credentials.
  fi
fi
