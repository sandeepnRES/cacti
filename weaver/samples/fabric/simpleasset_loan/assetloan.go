...
func (s *SmartContract) ClaimLoanRepayment(ctx contractapi.TransactionContextInterface, pledgeId, remoteNetworkId, remotePledgeBytes64 string) error {
	...
	token, err := getTokenAssetFromPledge(tokenPledgeBytes64)
	pledgeConditionFromTokenLedger, err := getLoanRepaymentConditionFromPledge(tokenPledgeBytes64)
	if err != nil {
		return err
	}
	
	pledgeConditionFromAssetLedger, err := getLoanRepaymentConditionFromPledge(remotePledgeBytes64)
	if err != nil {
		return err
	}
	asset, err := getBondAssetFromPledge(remotePledgeBytes64)
	if err != nil {
		return err
	}
...
// Since pledge is deleted in Step 7, hence we need to use claim status of
// Step 7 as proof for Step 8 claim.
func (s *SmartContract) ClaimLoanRepayment_old(ctx contractapi.TransactionContextInterface, pledgeId, remoteNetworkId, remoteClaimStatusBytes64 string) error {
	// (Optional) Ensure that this function is being called by the Fabric Interop CC
...
func (s *SmartContract) GetAssetLoanClaimStatus(ctx contractapi.TransactionContextInterface, pledgeId string) (string, error) {
	...
	claimAssetDetails, claimBytes64, blankClaimBytes64, err := wutils.GetAssetClaimStatus(ctx, pledgeId, pledge.Recipient, loanRepaymentCondition.TokenLedgerId, pledge.ExpiryTimeSecs, blankAssetJSON)
	if err != nil {
