...
func (s *SmartContract) PledgeRepaymentTokens(ctx contractapi.TransactionContextInterface, assetType string, numUnits uint64, remoteNetworkId, recipientCert string, expiryTimeSecs uint64, loanRepaymentConditionJSONStr string) (string, error) {
	...
	localNetworkId, err := ctx.GetStub().GetState(wutils.GetLocalNetworkIDKey())
	lenderECertBase64 := recipientCert
	...
	loanRepaymentCondition.TokenLedgerLenderCert = lenderECertBase64
	loanRepaymentCondition.TokenLedgerBorrowerCert = borrowerECertBase64
	
	loanRepaymentConditionJSON, err = json.Marshal(loanRepaymentCondition)

	// Pledge the asset using common (library) logic
	if pledgeId, err := wutils.PledgeAsset(ctx, assetJSON, assetType, strconv.Itoa(int(numUnits)), remoteNetworkId, recipientCert, expiryTimeSecs, loanRepaymentConditionJSON); err == nil {
...
func GetLoanRepaymentCondition(ctx contractapi.TransactionContextInterface, pledgeId string) (string, error) {
	_, assetPledgeBytes64, err := wutils.GetAssetPledgeDetails(ctx, pledgeId)
	pledgeConditionFromAssetLedger, err := getLoanRepaymentConditionFromPledge(assetPledgeBytes64)
	if err != nil {
		return "", err
