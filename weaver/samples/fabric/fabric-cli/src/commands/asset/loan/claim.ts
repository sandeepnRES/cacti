  ...
  run: async toolbox => {
    ...
    if (options.help || options.h) {
      commandHelp(
        print,
        toolbox,
        `fabric-cli asset loan claim --target-network=network1 --secret=secrettext --recipient=alice --contract-id=abc01`,
        'fabric-cli asset loan claim --fungible --target-network=<network1|network2> --hash_fn=<hash-function> --secret=<secret> --recipient=<recipient-userid> --contract-id=<contract-id> --locker=<locker-userid> --param=<param>',
        [
          {
            name: '--fungible',
            description:
