  ...
  run: async toolbox => {
    ...
    if (options.help || options.h) {
      commandHelp(
        print,
        toolbox,
        `fabric-cli asset loan lock --target-network=network1 --hashBase64=ivHErp1x4bJDKuRo6L5bApO/DdoyD/dG0mAZrzLZEIs= --timeout-duration=100 --locker=bob --recipient=alice --param=Type1:a04`,
        'fabric-cli asset loan lock --fungible --target-network=<network1|network2> --hash_fn=<hash-function-name> --hashBase64=<hashvalue-in-base64> --timeout-epoch=<timeout-epoch> --timeout-duration=<timeout-duration> --locker=<locker-userid> --recipient=<recipient-userid> --param=<param>',
        [
          {
            name: '--fungible',
            description:
