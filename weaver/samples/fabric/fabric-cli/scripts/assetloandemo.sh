...
./bin/fabric-cli asset loan lock --timeout-duration=3600 --locker=alice --recipient=bob --hashBase64=ivHErp1x4bJDKuRo6L5bApO/DdoyD/dG0mAZrzLZEIs= --target-network=network1 --param=${assettype}:${assetid} &> tmp.out
tmp=$(cat tmp.out | grep "Locked with Contract Id: " | sed -e 's/.*Locked with Contract Id: //')
CID=(${tmp//,/})
cat tmp.out
echo $CID

./bin/fabric-cli asset loan lock --fungible --timeout-duration=1800 --locker=bob --recipient=alice --hashBase64=ivHErp1x4bJDKuRo6L5bApO/DdoyD/dG0mAZrzLZEIs= --target-network=network2 --param=token1:${tokenamt} &> tmp.out
tmp=$(cat tmp.out | grep "Locked with Contract Id: " | sed -e 's/.*Locked with Contract Id: //')
CID2=(${tmp//,/})
cat tmp.out
echo $CID2

./bin/fabric-cli asset loan claim --fungible --recipient=alice --target-network=network2 --contract-id=$CID2 --secret=secrettext
...
./bin/fabric-cli asset loan claim-repayment --token-network=network2 --asset-network=network1 --borrower=alice --lender=bob --type=bond --pledge-id=$PID2 --asset-pledge-id=$PID
