 ...
 */
class ClaimFungibleAssetCommand : CliktCommand(help = "Claim a locked asset. Only Recipient's call will work.") {
    val config by requireObject<Map<String, String>>()
...
class ClaimAndPledgeCommand : CliktCommand(
        help = "Claim And Pledge an asset. claim-and-pledge --fungible --hashBase64=hashbase64 --timeout=10 --recipient='PartyA' --param=type:amount ") {
    val config by requireObject<Map<String, String>>()
    val hash_fn: String? by option("-hfn", "--hash-fn", help="Hash Function to be used. Default: SHA256")
    val hashBase64: String? by option("-h64", "--hashBase64", help="Hash in base64 for HTLC")
    val timeout: String? by option("-t", "--timeout", help="Timeout duration in seconds.")
